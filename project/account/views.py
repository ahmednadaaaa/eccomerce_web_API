...
def register(request):
    ...
    if user.is_valid(): #لو المعلومات صحيحه
        if not User.objects.filter(username=data['email']).exists(): # علشان لو ال ايميل مش موجود قبل كده ف الداتا بيز
            user = User.objects.create(
                first_name=data['first_name'],
                last_name=data['last_name'],
                email=data['email'],
                username=data['email'],
                password=make_password(data['password']),#make_password علشان يشفرها
            )
            return Response(
...
def update_user(request):
    ...
    if data['password'] != "": # لو مش فاضيه معناها انويريد تغير الباسورد
